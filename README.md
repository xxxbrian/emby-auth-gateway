...
| `GATEWAY_MEDIA_BUFFER_BUDGET` | No | automatic when buffering is enabled | Optional hard budget as a positive integer immediately followed by `B`, `KiB`, `MiB`, or `GiB`; the parsed value is aligned down to 32 KiB and must remain at least 32 KiB. Leave unset for automatic memory-limit discovery. It is ignored while buffering is disabled; an explicitly empty value is invalid when enabled. |
...
The PocketBase Admin UI is available at `http://localhost:8090/_/` when the gateway is running (keep `/_/` and `/api` on a private management origin in production).

### Admin control plane (always on)
...
```sh
docker pull ghcr.io/xxxbrian/emby-auth-gateway:0.8.0
