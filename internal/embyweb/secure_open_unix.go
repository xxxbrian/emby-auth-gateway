...
// openRegularFileNoFollow opens an absolute path without following a final-component
// symlink. Intermediate path components are still resolved by the kernel; prefer
// openAnchoredDir + Open for root-relative serve paths.
...
// nonblock after fstat confirms a regular file. Every path component is opened
// with O_NOFOLLOW so intermediate symlinks cannot escape the root.
func (d *anchoredDir) Open(rel string) (*os.File, error) {
	...
	if err != nil {
		...
		// Retry as directory without NONBLOCK if the first open failed.
...
func (d *anchoredDir) walkToParent(rel string) (parent int, base string, cleanup func(), err error) {
	...
	for _, seg := range parts[:len(parts)-1] {
		...
		if err != nil {
			cleanup()
			// O_NOFOLLOW may surface ELOOP, or ENOTDIR on some platforms when the
			// component is a symlink (directory open refuses to follow it).
			if errors.Is(err, unix.ELOOP) || isSymlinkAt(parent, seg) {
