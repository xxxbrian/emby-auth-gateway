...
func phase5DispatchServer(t *testing.T) (*Server, *MemoryStore, *phase5AuthSpy, *phase5MetadataSpy, *phase5MediaSpy) {
	t.Helper()
	...
	return server, store, auth, metadata, media
}
...
func TestPhase5ServerDispatchesOnlyToExactAdapter(t *testing.T) {
	tests := []struct {
		...
		{"metadata", http.MethodGet, "/System/Info", "metadata", http.StatusOK, 1},
		{"media image", http.MethodGet, "/Items/item/Images/Primary", "media", http.StatusOK, 1},
		{"playback info get", http.MethodGet, "/Items/item/PlaybackInfo", "negotiation", http.StatusOK, 1},
		{"playback info post", http.MethodPost, "/Items/item/PlaybackInfo", "negotiation", http.StatusOK, 1},
		{"live open", http.MethodPost, "/LiveStreams/Open", "negotiation", http.StatusOK, 1},
		{"live info", http.MethodPost, "/LiveStreams/MediaInfo", "negotiation", http.StatusOK, 1},
		{"live close", http.MethodPost, "/LiveStreams/Close", "negotiation", http.StatusOK, 1},
		{"encoding delete", http.MethodDelete, "/Videos/ActiveEncodings", "negotiation", http.StatusOK, 1},
		{"encoding delete compat", http.MethodPost, "/Videos/ActiveEncodings/Delete", "negotiation", http.StatusOK, 1},
		// Phase 8: unknown routes are Unclassified 404 with zero Ensure/adapters.
		{"unclassified", http.MethodGet, "/Unknown", "none", http.StatusNotFound, 0},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			server, _, auth, metadata, media := phase5DispatchServer(t)
			defer server.Close()
			var body io.Reader
			...
			if writer.Code != tt.wantStatus {
				t.Fatalf("status = %d body=%q, want %d", writer.Code, writer.Body.String(), tt.wantStatus)
			}
			got := map[string]int{"metadata": metadata.calls, "media": media.media, "negotiation": media.negotiation}
			...
			if auth.ensure != tt.wantEnsure {
				t.Fatalf("Ensure calls = %d, want %d", auth.ensure, tt.wantEnsure)
			}
		})
	}
}
...
func TestPhase5MetadataForeignSelectorDeniedBeforeEnsure(t *testing.T) {
	...
	writer := httptest.NewRecorder()
	server.ServeHTTP(writer, req)
	if writer.Code != http.StatusForbidden || auth.ensure != 0 || metadata.calls != 0 || media.media != 0 || media.negotiation != 0 {
		t.Fatalf("status=%d ensure=%d adapters=%d/%d/%d", writer.Code, auth.ensure, metadata.calls, media.media, media.negotiation)
	}
}

func TestPhase5LeaseLifecycleLogoutInactiveAndPeriodic(t *testing.T) {
	server, store, _, _, _ := phase5DispatchServer(t)
	defer server.Close()
...
func TestPhase5ServerAuditsRedirectDenial(t *testing.T) {
	// Redirect denial is observed on a curated metadata route that still proxies.
	var targetHits int
	...
	defer server.Close()
	writer := httptest.NewRecorder()
	server.ServeHTTP(writer, httptest.NewRequest(http.MethodGet, "http://gateway.test/emby/System/Info?api_key=gateway-token", nil))
...
func TestPhase5LeaseAndCapacityStatusMapping(t *testing.T) {
	// Use curated binary image MediaProxy path (stream is Unclassified until media-boundary lane).
	server, _, auth, metadata, media := phase5DispatchServer(t)
	defer server.Close()
	...
	server.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "http://gateway.test/emby/Items/item/Images/Primary?api_key=gateway-token&PlaySessionId=foreign-play", nil))
	if w.Code != http.StatusNotFound || auth.ensure != 0 || metadata.calls != 0 || media.media != 0 || media.negotiation != 0 {
		t.Fatalf("foreign status=%d ensure=%d adapters=%d/%d/%d", w.Code, auth.ensure, metadata.calls, media.media, media.negotiation)
	}

	server, _, auth, _, _ = phase5DispatchServer(t)
	defer server.Close()
	server.mediaUpstream = phase5FailMedia{err: ErrStoreUnavailable}
	w = httptest.NewRecorder()
	server.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "http://gateway.test/emby/Items/item/Images/Primary?api_key=gateway-token", nil))
...
func TestPhase5NegotiationOversizeRejectedBeforeEnsure(t *testing.T) {
	...
	if w.Code != http.StatusRequestEntityTooLarge || auth.ensure != 0 || metadata.calls != 0 || media.media != 0 || media.negotiation != 0 {
		t.Fatalf("status=%d ensure=%d adapters=%d/%d/%d", w.Code, auth.ensure, metadata.calls, media.media, media.negotiation)
	}
}
...
func TestServerCloseStopsLeaseMaintenanceIdempotently(t *testing.T) {
	server, _, _, _, _ := phase5DispatchServer(t)
